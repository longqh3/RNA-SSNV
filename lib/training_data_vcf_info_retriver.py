...
def case_vcf_info_retrive(tumor_tsv, case_id, vcf_folder_path, table_folder_path,
    ...
    try:
        ...
        # "AS_UNIQ_ALT_READ_COUNT", "ClippingRankSum",
        ...
        col_names = ['Chromosome', 'Start_Position', 'Reference_Allele', 'Tumor_Allele1',
                     ...
                     'record_filter',
                     "AS_UNIQ_ALT_READ_COUNT", "FS",
                     "SOR", "ROQ"]
        # 添加DNA tumor bam的测序深度/覆盖度信息 + GENCODE v22版本对应exon结构信息 + Funcotator注释信息
...
def record_select(record, case_id, RNA_tumor_aliquots_id, DNA_normal_aliquots_id,
                  samfile_list, exon_region_dict):
    """Retrieve all features based on given case_id.
    ...
    try:
        ...
        tmp_vcf_info = [record.CHROM, record.POS, record.REF, record.ALT[0],
                        ...
                        record.genotype(high_tumor_AD_aliquots_id).data.AF, record.genotype(high_normal_AD_aliquots_id).data.AF,
...
if __name__ == '__main__':
    ...
    tumor_tsv = pd.read_table(f"/home/lqh/Codes/Python/Integrative_Analysis_Bioinformatics_Pipeline/tables/{CANCER_TYPE}_RNA_somatic_calling_info.tsv")
    # 包含有DNA tumor相关注释信息的tsv文件，通常为WXS体细胞突变检测对应信息列表文件
    DNA_tumor_tsv = pd.read_table(f"/home/lqh/Codes/Python/Integrative_Analysis_Bioinformatics_Pipeline/tables/{CANCER_TYPE}_WXS_somatic_calling_info.tsv")
    ...
    # Output（修改相应路径信息即可）
    table_folder_path = f"/home/lqh/Codes/Python/Integrative_Analysis_Bioinformatics_Pipeline/results/{CANCER_TYPE}/RNA/RNA_somatic_mutation/VcfAssembly_new/SNP_WES_Interval_exon_updated"
