...
class exon_RNA_analysis_newer(object):
    ...
    def __init__(self, all_info, GDC_info, WES_info, RNA_edit_info):
        ...
        # self.all_info = self.new_feature_constructor(self.all_info)
        #
        # print("="*100)
        ...
        # # 找到RNA中有GDC数据支持的case
        # RNA_GDC_case_info_set = RNA_case_info_set.intersection(GDC_case_info_set)
        # print(f"确认GDC证据前，RNA突变数据集的case数为{len(RNA_case_info_set)}，"
        #       f"GDC突变数据集的case数为{len(GDC_case_info_set)}，"
        #       f"确认GDC证据后，具有GDC证据的RNA突变数据集的case数为{len(RNA_GDC_case_info_set)}")
        # # 对RNA突变数据集进行取子集操作，仅选择具有GDC证据的数据集
        # print("开始对RNA突变数据集取子集，仅选择具有GDC证据的case来进行分析")
        # print(f"取子集前，RNA突变数据集记录数目为{len(self.all_info)}")
        # self.all_info = self.all_info[self.all_info['Tumor_Sample_UUID'].isin(RNA_GDC_case_info_set)]
        # self.GDC_SNP_info = self.GDC_SNP_info[self.GDC_SNP_info['Tumor_Sample_UUID'].isin(RNA_GDC_case_info_set)]
        # print(f"取子集后，RNA突变数据集记录数目为{len(self.all_info)}")

        print(f"GDC数据集在根据RNA突变数据集信息取子集前，包含的case数目为{len(self.GDC_SNP_info['Tumor_Sample_UUID'].value_counts().index)}")
        self.GDC_SNP_info = self.GDC_SNP_info[self.GDC_SNP_info['Tumor_Sample_UUID'].isin(RNA_case_info_set)]
        print(f"GDC数据集在根据RNA突变数据集信息取子集后，包含的case数目为{len(self.GDC_SNP_info['Tumor_Sample_UUID'].value_counts().index)}")
        self.all_info_final = self.all_info.copy(deep=True)
        self.GDC_SNP_info_final = self.GDC_SNP_info.copy(deep=True)
        print("至此，纳入训练的RNA突变数据集和GDC突变数据集已确定，分别命名为all_info_final和GDC_SNP_info_final")
...
if __name__ == '__main__':
    ...
    # 初始化所有训练数据
    common_RF_exon = exon_RNA_analysis_newer(all_info, GDC_info, WES_info, RNA_EDIT_INFO)
